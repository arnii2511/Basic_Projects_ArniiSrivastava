...
mycursor.execute("CREATE TABLE IF NOT EXISTS exam_registration_system1(CANDIDATE_NAME CHAR(50),FATHER_NAME CHAR(50),MOTHER_NAME CHAR(50),AADHAR_NUMBER VARCHAR(20),NATIONALITY CHAR(20),GENDER_TYPE CHAR(7),CATEGORY_TYPE CHAR(10),EXAM_TYPE CHAR(10),CURRENT_ADDRESS VARCHAR(100),RESIDENCE_PLACE VARCHAR(100),CITY_NAME CHAR(20),DISTRICT_NAME CHAR(20),STATE_NAME CHAR(20),PIN_CODE VARCHAR(7),EMAIL_ADDRESS VARCHAR(30),PHONE_NUMBER VARCHAR(11),PASS_STATUS_10 CHAR(5),NAME_EXAM_10 CHAR(10),SCHOOL_BOARD_10 CHAR(10),PASS_YEAR_10 VARCHAR(5),PASS_STATUS_12 CHAR(5),NAME_EXAM_12 CHAR(10),SCHOOL_BOARD_12 CHAR(10),PASS_YEAR_12 VARCHAR(5));")
...
def next1():
    ...
    def mysqlconnectivity1():
            ...
            sql = "UPDATE exam_registration_system1 SET CURRENT_ADDRESS= %s,RESIDENCE_PLACE= %s,CITY_NAME= %s,DISTRICT_NAME= %s,STATE_NAME= %s,PIN_CODE= %s,EMAIL_ADDRESS= %s,PHONE_NUMBER = %s "
...
def next2():
    ...
    def mysqlconnectivity2():
            ...
            sql = "UPDATE exam_registration_system1 SET PASS_STATUS_10 = %s,NAME_EXAM_10 = %s,SCHOOL_BOARD_10 = %s,PASS_YEAR_10 = %s,PASS_STATUS_12 = %s,NAME_EXAM_12 = %s,SCHOOL_BOARD_12 = %s,PASS_YEAR_12 = %s "
...
def next3():
    ...
    def upi():
        ...
        F19=Frame(root4,bd=20,relief=RIDGE)
        F19.pack(side=TOP)

        label6=Label(F19,font=('arial',20,'bold'),text='EXAM   REGISTRATION   SYSTEM',bd=20,bg='powder blue',justify=CENTER,width=30, height=1)
        label6.grid(row=0)

        F20=Frame(root4,bd=15,relief=RIDGE)
        F20.pack(side=TOP)

        title6 = Label(F20, text="PAYMENT  DETAILS", font=('Calibri', 16, 'bold'),bd=20,bg='powder blue',width=35, height=1)
        title6.grid(row=0)

        F21=Frame(root4,bd=15,relief=RIDGE)
        F21.pack(side=TOP)

        upiid_var=StringVar()

        upiid = Label(F21, text='UPI ID:', font=("Calibri",20))
        upiid.grid(row=0,column=0)
        upi_id= Entry(F21,textvariable = upiid_var, font=('calibre',15,'normal'),bd=5,relief=SUNKEN)
        upi_id.grid(row=0,column=1,padx=10,pady=(10,5))

        T3=Text(F21, height =1, width = 12,font=('calibre',15,'normal'))
        T3.grid(row=2,column=1, padx=(0,30),pady=10)
        ...
        C6=Checkbutton(F21,text="Get Fees.",onvalue=1,offvalue=0,height=5,width=40,command=fees)
        C6.grid(row=3,column=0,pady=20)
        
        B7=Button(root4,font=('arial',16,'bold'),width=20,text='PAY',bg='white',command=successfull,state=DISABLED,relief=RIDGE,bd=10)
        B7.pack(side=BOTTOM)
...
def mysqlconnectivity():
    ...
    sql = "INSERT INTO exam_registration_system1(CANDIDATE_NAME,FATHER_NAME,MOTHER_NAME,AADHAR_NUMBER,NATIONALITY,GENDER_TYPE,CATEGORY_TYPE,EXAM_TYPE)VALUES (%s,%s,%s,%s,%s,%s,%s,%s)"
